import DefaultImg from "@/assets/profile-placeholder.png";
...
const HeadshotSection = ({ image, rotation, description, onDescriptionChange, onImageChange, onRotate, onRemove, onOpenModal, dragHandlers }) => {
  ...
  return (
    <Card className="p-8 mx-auto flex flex-col gap-[18px] bg-white rounded-lg border border-[#E2E4E9] shadow-none">
      ...
      <p className="text-sm font-inter flex gap-[4px] font-normal text-[#0A0D14]">
        ...
        Upload a smiling headshot of yourself, which you would also use for your student ID.
      </p>
      <p className="font-inter font-normal text-sm text-[#0A0D14]">
        Use a clear, front-facing photo with good lighting and a friendly smile. Avoid selfies, filters, hats, or anything inappropriate in the background.
      </p>
        ...
        <div className="w-full md:w-1/3 flex flex-col items-center">
          <div className="relative w-full h-[425px] overflow-hidden border border-gray-300">
            {currentImage || showDefaultImage ? (
              <img src={currentImage || defaultImageUrl} alt="Headshot" className="w-full h-full object-cover transition-transform duration-500" style={{ transform: `rotate(${rotation}deg)` }} />
            ) : (
